...
if (svgMatches.length === 1) {
  ...
  const methodAdvisories = collectMethodAdvisories(svg);
  const activeAdvisories = methodAdvisories.filter((entry) => !entry.waived);
  const advisoriesAreErrors = qualityProfile === 'showcase';
  const compositionErrors = (advisoriesAreErrors ? activeAdvisories.length : 0) + (qualityGatesEnforced ? containerBorderRuns.length : 0)
  ...
  const compositionWarnings = (advisoriesAreErrors ? 0 : activeAdvisories.length) + (qualityGatesEnforced ? 0 : containerBorderRuns.length)
  ...
  composition = {
    ...
    metrics: {
      ...
      methodAdvisories: activeAdvisories.length,
      waivedAdvisories: methodAdvisories.length - activeAdvisories.length,
      minProjectedNodeTextPx: desktopReadabilityIssue?.projectedFontPx ?? null,
      ...roundedRouteMetrics(routeMetrics),
    },
    suggestedLimits: { bendsPerRelationship: 2, stretch: 1.35, segmentPx: 16, microSegmentPx: 8 },
    issues: [
      ...methodAdvisories.map((entry) => ({
        severity: entry.waived ? 'waived' : advisoriesAreErrors ? 'error' : 'warning',
        code: `method/${entry.rule}`,
        rule: entry.rule,
        message: entry.message,
        ...(entry.subject ? { subject: entry.subject } : {}),
        ...(entry.waived ? { waiverReason: entry.waiverReason } : {}),
      })),
